...
import Data.Machine.Machine (MachineT(..), Machine, Step(..), unAwait, mkAwait, stopped, encased)
...
wye ma mb m = MachineT $ unwrap m >>= \v -> case v of
  Yield o k -> pure $ Yield o (wye ma mb k)
  Stop      -> pure Stop
  Await aw' -> aw' # unAwait \f wyin ff -> case wyin of
    WyeL lpf -> unwrap ma >>= \u -> case u of
      Yield a k -> unwrap <<< wye k mb $ f (coerceSymm lpf a)
      Stop       -> unwrap $ wye stopped mb ff
      Await awl  -> awl # unAwait \g (Refl rpfa) fg ->
        pure <<< mkAwait (\a -> wye (g $ coerceSymm rpfa $ a) mb $ encased v) (WyeL id) <<< wye fg mb $ encased v
    WyeR rpf -> unwrap mb >>= \u -> case u of
      Yield b k           -> unwrap <<< wye ma k $ f (coerceSymm rpf b)
      Stop                -> unwrap $ wye ma stopped ff
      Await awr           -> awr # unAwait' \g (Refl fpfa') fg ->
        pure <<< mkAwait (\b -> wye ma (g $ coerceSymm fpfa' $ b) $ encased v) (WyeR id) <<< wye ma fg $ encased v
    WyeE epf -> unwrap ma >>= \u -> case u of
      Yield a k -> unwrap <<< wye k mb $ f (coerceSymm epf (Left a))
      Stop      -> unwrap mb >>= \w -> case w of
        Yield b k   -> unwrap <<< wye stopped k <<< f $ coerceSymm epf (Right b)
        Stop        -> unwrap $ wye stopped stopped ff
        Await awre  -> awre # unAwait' \g (Refl arf) fg ->
          pure <<< mkAwait (\b -> wye stopped (g $ coerceSymm arf $ b) $ encased v) (WyeR id)
               <<< wye stopped fg $ encased v
      Await awle -> awle # unAwait \g (Refl alf) fg -> unwrap mb >>= \w -> case w of
        Yield b k   -> unwrap <<< wye (encased u) k <<< f $ coerceSymm epf $ Right b
        Stop        ->
          pure <<< mkAwait (\a -> wye (g $ coerceSymm alf $ a) stopped $ encased v) (WyeL id)
               <<< wye fg stopped $ encased v
        Await awe'  -> awe' # unAwait' \h (Refl hrf) fh ->
          pure <<< mkAwait (\c' -> case c' of
                                Left a  -> wye (g $ coerceSymm alf $ a) (encased w) $ encased v
                                Right b -> wye (encased u) (h $ coerceSymm hrf $ b) $ encased v) (WyeE id)
               <<< wye fg fh $ encased v
